...
const TetrisBoard: React.FC<TetrisBoardProps> = (props) => {
  ...
  props.field.forEach((row, index) => {
    ...
    const cols = row.map((column: any, index: number) => (
      <div className={`col-tetris-${column}`} key={index} />
    ));
  ...
  return (
    <div className="tetris-board">
      {/* Game info */}
      <div className="tetris-board__info" style={{ background: "black" }}>
        <p className="tetris-board__text">Nivel: {props.level}</p>

        <p className="tetris-board__text">Puntaje: {props.score}</p>
